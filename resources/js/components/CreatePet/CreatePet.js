...
export default class  PostEntry extends React.Component
{
    ...
    handleSubmit = (event)=>{
        ...
        const {species,breed,gender,age,name, description, image} = this.state;
        let newPet;
        if (image === "")
        {
            newPet = {
        ...
        let url = `/api/pet/create/${id}?token=${this.state.token}`;
        ...
        .then(
            (response) => {
                ...
                this.props.onSuccess(`${this.props.user.name} upload a pet ${name}!`);
                this.setState({
                    species: species,
                    breed: "",
                    gender: "",
